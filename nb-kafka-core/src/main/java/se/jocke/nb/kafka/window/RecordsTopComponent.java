...
import se.jocke.nb.kafka.nodes.record.NBKafkaRecordNode;
...
public final class RecordsTopComponent extends TopComponent implements ExplorerManager.Provider {
  ...
  public RecordsTopComponent() {
    ...
    Lookup lkp = new ProxyLookup(new AbstractLookup(content), ExplorerUtils.createLookup(getExplorerManager(), getActionMap()));
    associateLookup(lkp);
    initComponents();
    setName("Records");
  ...
  protected void handleSelectionEvent(ListSelectionEvent e) {
    ...
    RecordTableModel model = (RecordTableModel) recordTable.getModel();
    NBKafkaConsumerRecord record = model.getRecord(e.getFirstIndex());
    if (record != null) {
      final NBKafkaRecordNode nbKafkaRecordNode = new NBKafkaRecordNode(record);
      getExplorerManager().setRootContext(nbKafkaRecordNode);
      getExplorerManager().setExploredContext(nbKafkaRecordNode);
      try {
        getExplorerManager().setSelectedNodes(new Node[]{nbKafkaRecordNode});
